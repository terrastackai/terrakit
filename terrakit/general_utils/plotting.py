...
def plot_cordex_variable(dataset, variable_name, time_index=0):
    """
    Plot a single variable from a CORDEX dataset with spatial map and optional profile.
    Handles both datasets with time dimension and single-date datasets without time dimension.
    Works with rotated pole coordinates (rlat/rlon) typical of CORDEX data.
    ...
    if "rlat" in var_data.dims and "rlon" in var_data.dims:
        lat_name = "rlat"
        lon_name = "rlon"
    elif "latitude" in var_data.dims and "longitude" in var_data.dims:
        lat_name = "latitude"
        lon_name = "longitude"
        ...
        # Fallback to any spatial dimensions
        spatial_dims = [d for d in var_data.dims if d not in ["time", "valid_time"]]
        if len(spatial_dims) >= 2:
            lat_name = spatial_dims[0]
            lon_name = spatial_dims[1]
        else:
            raise ValueError(
                f"Could not identify spatial dimensions in variable {variable_name}"
            )
...
def plot_cordex_variables_comparison(dataset, time_index=0):
    """
    Plot all variables from a CORDEX dataset side by side with spatial maps.
    Handles both datasets with time dimension and single-date datasets without time dimension.
    Works with rotated pole coordinates (rlat/rlon) typical of CORDEX data.
    ...
    # Identify spatial dimensions - CORDEX typically uses rlat/rlon
    if "rlat" in first_var.dims and "rlon" in first_var.dims:
        lat_name = "rlat"
        lon_name = "rlon"
    elif "latitude" in first_var.dims and "longitude" in first_var.dims:
        lat_name = "latitude"
        lon_name = "longitude"
    elif "lat" in first_var.dims and "lon" in first_var.dims:
        lat_name = "lat"
        lon_name = "lon"
    else:
        # Fallback to any spatial dimensions
        spatial_dims = [d for d in first_var.dims if d not in ["time", "valid_time"]]
        if len(spatial_dims) >= 2:
            lat_name = spatial_dims[0]
            lon_name = spatial_dims[1]
        else:
            raise ValueError("Could not identify spatial dimensions in dataset")
    ...
    for v_idx, var_name in enumerate(variables):
        ...
        if n_lat > 1 and n_lon > 1:
            ...
            data.plot.pcolormesh(ax=ax, cmap="viridis", add_colorbar=True)
            ax.set_xlabel(lon_name.capitalize())
            ax.set_ylabel(lat_name.capitalize())
        elif n_lat > 1 or n_lon > 1:
            # 1D profile
            data.plot(ax=ax, marker="o")
            ax.set_ylabel(f"{var_data.attrs.get('units', 'Value')}")
