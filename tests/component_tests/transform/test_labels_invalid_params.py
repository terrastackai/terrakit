...
class TestLabels_Classes:
    def test_process_labels__with_class_zero(
        self,
        process_labels_clean_up_working_dir,
        caplog,
    ):
        """Test that process_labels correctly handles multi-class labels including class 0.

        This test verifies that labels with class 0 and class 1 are processed correctly.
        The actual validation of class 0 conflict with set_no_data=False is tested in
        test_download_data.py::TestDownloadData_SetNoDataWithClasses::test_download_data__class_zero_conflict_raises_error
        """
        # Process labels with class 0 and class 1
        labels_gdf, grouped_boxes_gdf = process_labels(
        ...
        # Verify shapefiles were created
        num_files = (
            2 * 5 + 1
